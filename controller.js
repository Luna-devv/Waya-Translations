module.exports = class I18n {
    ...
    getData(type, language, command) {
        try {
            ...
            return require(`./languages/${language || this.options.language.toLowerCase()
    ...
    get(path, replace, plural, fallback) {
        ...
        if (fallback || !data || Object?.keys(data)?.length == 0)
            data = this.fallback;
        if (!this.fallback) data = {};

        data._global = this._global;
        if (fallback || !data._global || Object?.keys(data._global) == 0)
